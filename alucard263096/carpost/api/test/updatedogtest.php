...
////以下是代码开始，请勿删除此行注释
////starthere
while(true){        $sql="select id,url from tb_dogtest where content='' limit 0,100";    $result=$dbmgr->fetch_array_all($dbmgr->query($sql));    if(count($result)==0){        die("done");    }    for($i=0;$i<count($result);$i++){                $url=$result[$i]["url"];        $id=$result[$i]["id"]+0;        $url=str_replace("&quot;","%22",$url);        $url=str_replace("&amp;","&",$url);                $content=parameter_filter( request_get($url));        $content=str_replace("callback(","",$content);        $content=substr($content,0,count($content)-2);        $sql="update tb_dogtest set content='$content' where id=$id ";        $dbmgr->query($sql);    }}
