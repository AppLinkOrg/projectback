...
////以下是代码开始，请勿删除此行注释
////starthere
$inst=$dbmgr->fetch_array($dbmgr->query("select * from tb_inst"));$app_id=$inst["appid"];$app_secret=$inst["appsecret"];$id=$_REQUEST["id"]+0;include USER_ROOT."common/wechat.cls.php";$wechatmgr=new WechatMgr($app_id,$app_secret);//print_r($config); $accesstoken=$wechatmgr->getAccessToken();$arr["path"]="/pages/info/info?id=$id";$arr["width"]="100";$arr=json_encode($arr);$res=$wechatmgr->httpPost("https://api.weixin.qq.com/wxa/getwxacode?access_token=$accesstoken",$arr);header("content-type:image/png");echo $res;exit;
