...
////以下是代码开始，请勿删除此行注释
////starthere
$post_id=$_REQUEST["post_id"]+0;if($dbmgr->checkHave("tb_post_follow"," post_id=$post_id and $member_id=member_id ")){    $dbmgr->query("delete from tb_post_follow  where post_id=$post_id and $member_id=member_id  ");outputJson(outResult(0,"delete",false));}else{    $dbmgr->query("insert into tb_post_follow  select ifnull(max(id),0)+1,now(),1,now(),1,$post_id,$member_id,now() from tb_post_follow ");outputJson(outResult(0,"add",true));}
