...
////以下是代码开始，请勿删除此行注释
////starthere
$center_lat=$_REQUEST["center_lat"]+0;$center_lng=$_REQUEST["center_lng"]+0;$post_id=$_REQUEST["post_id"]+0;$post=$dbmgr->fetch_array($dbmgr->query("select *,(((lat-$center_lat)*(lat-$center_lat))+((lng-$center_lng)*(lng-$center_lng))) distancefrom tb_post where id=$post_id "));$distance=$post["distance"]+0;$nextpost=$dbmgr->fetch_array($dbmgr->query("select * from (select *,(((lat-$center_lat)*(lat-$center_lat))+((lng-$center_lng)*(lng-$center_lng))) distancefrom tb_post where status='A') aa where distance>$distance order by distance"));$nextpost_id=$nextpost["id"]+0;if($nextpost_id>0){    outputJson(outResult(0,"success",$nextpost_id));}else{    $nextpost=$dbmgr->fetch_array($dbmgr->query("select *,(((lat-$center_lat)*(lat-$center_lat))+((lng-$center_lng)*(lng-$center_lng))) distancefrom tb_post where status='A' order by distance "));$nextpost_id=$nextpost["id"]+0;outputJson(outResult(0,"success",$nextpost_id));}
