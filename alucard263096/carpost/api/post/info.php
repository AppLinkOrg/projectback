...
////以下是代码开始，请勿删除此行注释
////starthere
/**************    请在此处编写请求的数据正确性，请求合法性的判断    比如:    if($_REQUEST["unicode"]=="")//编码    {        outputJSON(Array());    }    ****************/    //1. （请勿改动的代码）获取数据模型对象    $modelname="post";    $modelmgrpath=USER_ROOT."modelmgr/$modelname.model.php";    $model=new XmlModel($modelname,CURRENT_PATH);    //修正$model的字段默认值，for($model->XmlData["fields"]["field"] as $field)    /**************    请在此处编写你对请求值的修正    $unicode=parameter_filter($_REQUEST["unicode"]);    $query=$dbmgr->query("select id from tb_context where unicode='$unicode'");    $result=$dbmgr->fetch_array($query);    $id=$result["id"]+0;    ****************/    $id=$_REQUEST["id"]+0;        $dbmgr->query("insert into tb_post_view  select ifnull(max(id),0)+1,now(),1,now(),1,$id,$member_id,now() from tb_post_view ");    //2. （请勿改动的代码）获取生成的sql语句    $sql=$model->GetSearchSqlField($request,true)." where r_main.id=$id";            $query = $dbmgr->query($sql);    $result = $dbmgr->fetch_array($query);    /**************    请在此处编写你对最终返回数据的修正    如    $result["mobile"]=substr($result[$i]["mobile"],0,2)."******".substr($result[$i]["mobile"],8,3);    ****************/        $result["liked"]=$dbmgr->checkHave("tb_post_like","  member_id=$member_id and post_id=$id");    $r=$dbmgr->fetch_array($dbmgr->query("select count(1) c from tb_post_like where post_id=$id "));    $result["likecount"]=$r["c"]+0;        $result["followed"]=$dbmgr->checkHave("tb_post_follow","  member_id=$member_id and post_id=$id");    $r=$dbmgr->fetch_array($dbmgr->query("select count(1) c from tb_post_follow where post_id=$id "));    $result["followcount"]=$r["c"]+0;        $r=$dbmgr->fetch_array($dbmgr->query("select count(1) c from tb_post_view where post_id=$id "));    $result["viewcount"]=$r["c"]+0;        $result["ismy"]=$result["member_id"]==$member_id;        request_get("http://cmsdev.app-link.org/alucard263096/petfind/api/post/poster?post_id=".$id);            outputJSON($result);
