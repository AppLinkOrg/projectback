...
////以下是代码开始，请勿删除此行注释
////starthere
//$page=$_REQUEST["latest"];
$new=date('Y-m-d H:i:s',strtotime($_REQUEST["new"]));
$latest=date('Y-m-d H:i:s',strtotime($_REQUEST["latest"]));
$openid=$_REQUEST["openid"];
$member_id=getMemberIdByOpenId($openid)+0;

$sql="select distinct help.id,help.photo,help.published_date,help.title,help.description from tb_help help inner join tb_helpup helpup on help.id=helpup.help_id  where helpup.member_id=$member_id";
if($new!="1970-01-01 08:00:00"){
$sql.=" and published_date>'$new' order by published_date desc";
}elseif($latest!="1970-01-01 08:00:00"){
$sql.=" and published_date<'$latest' order by  published_date desc limit 0,3";
}else{
$sql.=" order by  published_date desc limit 0,3";
}

// order by published_date desc";
$result=$dbmgr->fetch_array_all($dbmgr->query($sql));
