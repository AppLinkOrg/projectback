...
////以下是代码开始，请勿删除此行注释
////starthere
$code=$_REQUEST["code"];$grant_type=$_REQUEST["grant_type"];if($code==""){outputJson(outResult(-101,"no code request, sorry"));}if($grant_type==""){outputJson(outResult(-102,"no grant_type request, sorry"));}$appid=$inst["appid"];$appsecret=$inst["appsecret"];$url="https://api.weixin.qq.com/sns/jscode2session?appid=$appid&secret=$appsecret&js_code=$code&grant_type=$grant_type";$authinfo=request_get($url);$authinfo=json_decode($authinfo,true);//$wechatmgr=new WechatMgr($appid,$appsecret);//$autoinfo=$wechatmgr->getUserBaseInfo($code);$authinfo["appid"]=$inst["appid"];$authinfo["appsecret"]=$inst["appsecret"];outputJson($authinfo);
