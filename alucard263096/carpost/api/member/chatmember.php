...
////以下是代码开始，请勿删除此行注释
////starthere
/**************    请在此处编写请求的数据正确性，请求合法性的判断    比如:    if($_REQUEST["mobile"]=="")//手机号码为空    {        outputJSON(Array());    }        ****************/    //1. （请勿改动的代码）获取数据模型对象    $modelname="member";    $modelmgrpath=USER_ROOT."modelmgr/$modelname.model.php";    $model=new XmlModel($modelname,CURRENT_PATH);        //修正$model的字段默认值，for($model->XmlData["fields"]["field"] as $field)    /**************    请在此处编写你对请求值的修正    如$request["request_time"]=date('Y-m-d H:i:s');    ****************/    $request=$_REQUEST;    $request["orderby"]="a.sent_time desc";        //2. （请勿改动的代码）获取自动生成的搜索sql语句    $sql=$model->GetSearchSql($request);    //echo $sql;      $sql=str_replace("select ","select distinct ",$sql);        $sql=str_replace("where ","inner join tb_member_chat a on r_main.id=a.sent_member_id or r_main.id=a.received_member_id     where (a.sent_member_id=$member_id or a.received_member_id=$member_id) and r_main.id<>$member_id and ",$sql);    $query = $dbmgr->query($sql);    $result = $dbmgr->fetch_array_all($query);     /**************    请在此处编写你对最终返回数据的修正    如    ****************/    for($i=0;$i<count($result);$i++){        $id=$result[$i]["id"]+0;        $c=$dbmgr->fetch_array($dbmgr->query("select * from tb_member_chat         where ((sent_member_id=$id and received_member_id=$member_id) or (sent_member_id=$member_id and received_member_id=$id) )        order by sent_time desc limit 0,1"));        $c["sent_time_timespan"]=strtotime($c["sent_time"]);        $c["ismysent"]=$c["sent_member_id"]==$member_id;        $result[$i]["chat"]=$c;    }    outputJSON($result);
