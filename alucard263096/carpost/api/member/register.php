...
////以下是代码开始，请勿删除此行注释
////starthere
require USER_ROOT."/common/aliyunmns.inc.php";$mobile=parameter_filter($_REQUEST["mobile"]);$name=parameter_filter($_REQUEST["name"]);$verifycode=parameter_filter($_REQUEST["verifycode"]);if($mobile==""){    outputJson(outResult(-403,"mobile cannot be null"));}//if($dbmgr->checkHave("tb_member"," $mobile='$mobile' and clinic_id=$clinic_id ")){//    $result=$dbmgr->fetch_array($dbmgr->query("select * from tb_member where openid='$openid' and clinic_id=$clinic_id "));//    outputJson(outResult(1,"opeid has been register",$result));//}if(1==1||SMSCodeVerify($mobile,$verifycode,"register")){    $dbmgr->begin_trans();         $sql="update tb_member set mobile='$mobile',name='$name'        where  id=$member_id  ";        $dbmgr->query($sql);        $dbmgr->commit_trans();        $result=$dbmgr->fetch_array($dbmgr->query("select * from tb_member where id=$member_id  "));    outputJson(outResult(0,"SUCCESS",$result));                }else{    outputJson(outResult(-501,"INVALID_VERIFYCODE"));}
