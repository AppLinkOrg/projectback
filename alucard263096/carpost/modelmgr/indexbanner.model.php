...
class indexbannerXmlModel extends XmlModel
{
  ...
  public function resetRequestData($dbMgr,$request){
    $user=getCMSSession("SysUser");
	if($user["role_id"]>0){
		$path=$this->getModelField("path");
		$inst_id=$user["inst_id"];
		$path["condition"].=" and objs.inst_id= $inst_id ";
		$this->setModelField("path",$cat_id);
	}
    return $request;
  }
